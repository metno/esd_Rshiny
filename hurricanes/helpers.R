...
prep.data <- function(fname="trajectories.hurdat2.rda",path=NULL,force=FALSE,verbose=TRUE) {
  ...
  if(!file.exists(fname) | force) {
    if(verbose) print("read or download HURDAT2 data")
    x <- read.hurdat2(verbose=verbose)
    Z <- as.trajectory(x,verbose=verbose)
    if(verbose) print("save trajectories to rda file")
    save(file=fname,Z)
  } else {
    if(verbose) print("load trajectories from rda file")
    load(fname)
  }
  invisible(Z)
}

map.fancy <- function(Y,param=NULL,xlim=c(-120,40),ylim=c(0,65)) {
  data(etopo5)
...
switch.param <- function(param) {
  if(grepl("bjornholt",tolower(param))) {
    ...
    param <- switch(param,"Pressure"="pcent","Month"="month","Year"="year",
                    "ENSO"="enso","NAO"="nao","AMO"="amo","Global temperature"="t2m",
                    "Maximum wind speed"="max.speed","Uniform"=NULL)
  }
  return(param)
}
